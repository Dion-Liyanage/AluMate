...
export function QuotationsOverviewCards() {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setStatsData({
          ...
          pending: quotations.filter(q => q.status === 'pending_approval' || q.status === 'pending').length,
          approved: quotations.filter(q => q.status === 'approved' || q.status === 'accepted').length,
          rejected: quotations.filter(q => q.status === 'rejected' || q.status === 'declined').length,
        ...
        <CardHeader className="relative pb-4">
          <div>
            ...
            <CardDescription className="text-zinc-400 mt-1">
              Review your fabrication estimates and approval statuses.
            </CardDescription>
          </div>
        </CardHeader>
